...
it("plays the move onto the position the game stands at", () => {
  ...
  expect(playMove(playedGameFrom(state), move(1, 10, 1, 5)).present).toEqual(applyMove(state, move(1, 10, 1, 5)));
});

it("keeps the position the move was played from", () => {
  const state = board();

  expect(playMove(playedGameFrom(state), move(1, 10, 1, 5)).past).toEqual([state]);
});

it("leaves nothing to play again, a move being the newest thing in the game", () => {
  const played = playMove(playedGameFrom(board()), move(1, 10, 1, 5));
...
it("forgets a move that had been taken back", () => {
  const played = undo(playMove(playedGameFrom(board()), move(1, 10, 1, 5)));

  expect(playMove(played, move(1, 10, 1, 8)).future).toEqual([]);
});

it("leaves the record it was handed untouched, so a game stays replayable", () => {
  const played = playedGameFrom(board());

  playMove(played, move(1, 10, 1, 5));
...
it("refuses a move the rules do not allow, in the engine's own words", () => {
  ...
  expect(() => playMove(played, move(1, 10, 2, 9))).toThrow("cannot move");
});

it("refuses to move a piece belonging to the other army", () => {
  const played = playedGameFrom(board());

  expect(() => playMove(played, move(5, 2, 5, 3))).toThrow("cho to move");
});
...
it("refuses once the game is over", () => {
  ...
  expect(() => playMove(played, move(1, 10, 1, 5))).toThrow("The game is over");
});
